...
if [[ "$syslog_daemon" == "rsyslog" ]]; then
  sudo systemctl stop rsyslog ; sleep 55 ; sudo systemctl start rsyslog ; sudo systemctl status rsyslog
  echo "rsyslog daemon restarted successfully."
elif [[ "$syslog_daemon" == "syslog-ng" ]]; then
  sudo systemctl stop syslog-ng ; sleep 55 ; sudo systemctl start syslog-ng ; sudo systemctl status syslog-ng
